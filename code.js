...
const tebiejie11botRegexp = /(tebiejie1_\d+V_\d+P_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie12botRegexp = /(tebiejie1_\d+V_\d+P_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie13botRegexp = /(tebiejie1_\d+V_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie14botRegexp = /(tebiejie1_\d+P_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie15botRegexp = /(tebiejie1_\d+V_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie16botRegexp = /(tebiejie1_\d+P_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie17botRegexp = /(tebiejie1_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie1bot
const tebiejie31botRegexp = /(tebiejie3bot_\d+V_\d+P_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie3bot
const tebiejie32botRegexp = /(tebiejie3bot_\d+V_\d+P_[A-Z0-9]{10,22})/g;   //tebiejie3bot
const tebiejie33botRegexp = /(tebiejie3bot_\d+V_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie3bot
const tebiejie34botRegexp = /(tebiejie3bot_\d+P_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie3bot
const tebiejie35botRegexp = /(tebiejie3bot_\d+V_[A-Z0-9]{10,22})/g;   //tebiejie3bot
const tebiejie36botRegexp = /(tebiejie3bot_\d+P_[A-Z0-9]{10,22})/g;   //tebiejie3bot
const tebiejie37botRegexp = /(tebiejie3bot_\d+D_[A-Z0-9]{10,22})/g;   //tebiejie3bot
...
const MessengerIIbotRegexp = /(MessengerIIbot_[A-Z0-9]{4}-[A-Z0-9]{4}_\d+P\d+V\d+D)/g;   //MessengerIIbot
const XVNgkllbotRegexp = /(XVNgkllbot_[A-Z0-9]{15})/g;   //XVNgkllbot
...
const FilesHubRobot1Regexp = /(FilesHub_Robot_[a-z0-9]{20})/g;   //FilesHubRobot
const FilesHubRobot2Regexp = /(Files_Hub_RoBot_[a-z0-9]{20})/g;   //FilesHubRobot
...
const yyjmq1Regexp = /(yyjmq-[a-z0-9]{16}_\d*P\d*V\d*D)/g;   //yyjmq
const yyjmq2Regexp = /(yyjmq-[a-z0-9]{16}_\d*P\d*V)/g;   //yyjmq
const yyjmq3Regexp = /(yyjmq-[a-z0-9]{16}_\d*P\d*D)/g;   //yyjmq
const yyjmq4Regexp = /(yyjmq-[a-z0-9]{16}_\d*V\d*D)/g;   //yyjmq
const yyjmq5Regexp = /(yyjmq-[a-z0-9]{16}_\d*P)/g;   //yyjmq
const yyjmq6Regexp = /(yyjmq-[a-z0-9]{16}_\d*V)/g;   //yyjmq
const yyjmq7Regexp = /(yyjmq-[a-z0-9]{16}_\d*D)/g;   //yyjmq
const yyjmqhbot1Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*P\d*V\d*D)/g;   //yyjmqhbot
const yyjmqhbot2Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*P\d*V)/g;   //yyjmqhbot
const yyjmqhbot3Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*P\d*D)/g;   //yyjmqhbot
const yyjmqhbot4Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*V\d*D)/g;   //yyjmqhbot
const yyjmqhbot5Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*P)/g;   //yyjmqhbot
const yyjmqhbot6Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*V)/g;   //yyjmqhbot
const yyjmqhbot7Regexp = /(yyjmqh_bot:[a-z0-9]{16}_\d*D)/g;   //yyjmqhbot
const aotembotRegexp = /(aotem_bot:[\-_A-Za-z0-9]{16})/g;   //aotembot
const jiematopbotRegexp = /(jiematopbot-[A-Za-z0-9]{18})/g;   //jiematopbot
...
const bffilesbot1Regexp = /(bffilesbot_[A-Za-z0-9]{20})/g;   //bffilesbot
const bffilesbot2Regexp = /(bffilesbot_\d+P_[A-Za-z0-9]{20})/g;   //bffilesbot
const bffilesbot3Regexp = /(bffilesbot_\d+V_[A-Za-z0-9]{20})/g;   //bffilesbot
const bffilesbot4Regexp = /(bffilesbot_\d+D_[A-Za-z0-9]{20})/g;   //bffilesbot
const KFCMNBbot1Regexp = /(KFCMNB_bot_\d+P_\d+V_\d+D_[A-Z0-9]{16})/g;   //KFCMNBbot
const KFCMNBbot2Regexp = /(KFCMNB_bot_\d+P_\d+V_[A-Z0-9]{16})/g;   //KFCMNBbot
const KFCMNBbot3Regexp = /(KFCMNB_bot_\d+P_\d+D_[A-Z0-9]{16})/g;   //KFCMNBbot
const KFCMNBbot4Regexp = /(KFCMNB_bot_\d+V_\d+D_[A-Z0-9]{16})/g;   //KFCMNBbot
const KFCMNBbot5Regexp = /(KFCMNB_bot_\d+P_[A-Z0-9]{16})/g;   //KFCMNBbot
const KFCMNBbot6Regexp = /(KFCMNB_bot_\d+V_[A-Z0-9]{16})/g;   //KFCMNBbot
const KFCMNBbot7Regexp = /(KFCMNB_bot_\d+D_[A-Z0-9]{16})/g;   //KFCMNBbot
const JSfilebotRegexp = /(JSfile_bot_\d+V\d+P\d+D_[A-Z0-9]{4}-[A-Z0-9]{4})/g;   //JSfilebot
const JSfilesbotRegexp = /(JSfilesbot_[A-Z0-9]{11})/g;   //JSfilesbot
const JSfilessbotRegexp = /(JSfilessbot_[A-Z0-9]{10})/g;   //JSfilessbot
const JSfileessbotRegexp = /(JSfileessbot_[A-Z0-9]{13})/g;   //JSfileessbot
const JSfileeesbotRegexp = /(JSfileeesbot_[A-Z0-9]{15})/g;   //JSfileeesbot
const JScodefilebotRegexp = /(JScodefilebot_[A-Z0-9]{14})/g;   //JScodefilebot
...
const amumushuangxiangbotRegexp = /(amumushuangxiangbot_palu_[A-Za-z0-9]{10})/g;   //amumushuangxiangbot
...
try {
  ...
  if (length > 1) {
    for (let i = 0; i < length; i++) {
      ...
      if (str) {
        ...
        const tebiejie11botMatches = str.match(tebiejie11botRegexp);
        // console.log(tebiejie11botMatches);  //测试
        if (tebiejie11botMatches) {
          const tebiejie11botMatchesLength = tebiejie11botMatches.length;
          // console.log("tebiejie11botMatchesLength : " + tebiejie11botMatchesLength);  //测试
          if (tebiejie11botMatchesLength > 0) {
            for (let j = 0; j < tebiejie11botMatchesLength; j++) {
              if (tebiejie11botMatches[j]) {
                tebiejiebot.push(tebiejie11botMatches[j]);
              }
            }
          }
        }

        const tebiejie12botMatches = str.match(tebiejie12botRegexp);
        // console.log(tebiejie12botMatches);  //测试
        if (tebiejie12botMatches) {
          const tebiejie12botMatchesLength = tebiejie12botMatches.length;
          // console.log("tebiejie12botMatchesLength : " + tebiejie12botMatchesLength);  //测试
          if (tebiejie12botMatchesLength > 0) {
            for (let j = 0; j < tebiejie12botMatchesLength; j++) {
              if (tebiejie12botMatches[j]) {
                tebiejiebot.push(tebiejie12botMatches[j]);
              }
            }
          }
        }

        const tebiejie13botMatches = str.match(tebiejie13botRegexp);
        // console.log(tebiejie13botMatches);  //测试
        if (tebiejie13botMatches) {
          const tebiejie13botMatchesLength = tebiejie13botMatches.length;
          // console.log("tebiejie13botMatchesLength : " + tebiejie13botMatchesLength);  //测试
          if (tebiejie13botMatchesLength > 0) {
            for (let j = 0; j < tebiejie13botMatchesLength; j++) {
              if (tebiejie13botMatches[j]) {
                tebiejiebot.push(tebiejie13botMatches[j]);
              }
            }
          }
        }

        const tebiejie14botMatches = str.match(tebiejie14botRegexp);
        // console.log(tebiejie14botMatches);  //测试
        if (tebiejie14botMatches) {
          const tebiejie14botMatchesLength = tebiejie14botMatches.length;
          // console.log("tebiejie14botMatchesLength : " + tebiejie14botMatchesLength);  //测试
          if (tebiejie14botMatchesLength > 0) {
            for (let j = 0; j < tebiejie14botMatchesLength; j++) {
              if (tebiejie14botMatches[j]) {
                tebiejiebot.push(tebiejie14botMatches[j]);
              }
            }
          }
        }

        const tebiejie15botMatches = str.match(tebiejie15botRegexp);
        // console.log(tebiejie15botMatches);  //测试
        if (tebiejie15botMatches) {
          const tebiejie15botMatchesLength = tebiejie15botMatches.length;
          // console.log("tebiejie15botMatchesLength : " + tebiejie15botMatchesLength);  //测试
          if (tebiejie15botMatchesLength > 0) {
            for (let j = 0; j < tebiejie15botMatchesLength; j++) {
              if (tebiejie15botMatches[j]) {
                tebiejiebot.push(tebiejie15botMatches[j]);
              }
            }
          }
        }

        const tebiejie16botMatches = str.match(tebiejie16botRegexp);
        // console.log(tebiejie16botMatches);  //测试
        if (tebiejie16botMatches) {
          const tebiejie16botMatchesLength = tebiejie16botMatches.length;
          // console.log("tebiejie16botMatchesLength : " + tebiejie16botMatchesLength);  //测试
          if (tebiejie16botMatchesLength > 0) {
            for (let j = 0; j < tebiejie16botMatchesLength; j++) {
              if (tebiejie16botMatches[j]) {
                tebiejiebot.push(tebiejie16botMatches[j]);
              }
            }
          }
        }

        const tebiejie17botMatches = str.match(tebiejie17botRegexp);
        // console.log(tebiejie17botMatches);  //测试
        if (tebiejie17botMatches) {
          const tebiejie17botMatchesLength = tebiejie17botMatches.length;
          // console.log("tebiejie17botMatchesLength : " + tebiejie17botMatchesLength);  //测试
          if (tebiejie17botMatchesLength > 0) {
            for (let j = 0; j < tebiejie17botMatchesLength; j++) {
              if (tebiejie17botMatches[j]) {
                tebiejiebot.push(tebiejie17botMatches[j]);
              }
            }
          }
        }

        const tebiejie31botMatches = str.match(tebiejie31botRegexp);
        // console.log(tebiejie31botMatches);  //测试
        if (tebiejie31botMatches) {
          const tebiejie31botMatchesLength = tebiejie31botMatches.length;
          // console.log("tebiejie31botMatchesLength : " + tebiejie31botMatchesLength);  //测试
          if (tebiejie31botMatchesLength > 0) {
            for (let j = 0; j < tebiejie31botMatchesLength; j++) {
              if (tebiejie31botMatches[j]) {
                tebiejiebot.push(tebiejie31botMatches[j]);
              }
            }
          }
        }

        const tebiejie32botMatches = str.match(tebiejie32botRegexp);
        // console.log(tebiejie32botMatches);  //测试
        if (tebiejie32botMatches) {
          const tebiejie32botMatchesLength = tebiejie32botMatches.length;
          // console.log("tebiejie32botMatchesLength : " + tebiejie32botMatchesLength);  //测试
          if (tebiejie32botMatchesLength > 0) {
            for (let j = 0; j < tebiejie32botMatchesLength; j++) {
              if (tebiejie32botMatches[j]) {
                tebiejiebot.push(tebiejie32botMatches[j]);
              }
            }
          }
        }

        const tebiejie33botMatches = str.match(tebiejie33botRegexp);
        // console.log(tebiejie33botMatches);  //测试
        if (tebiejie33botMatches) {
          const tebiejie33botMatchesLength = tebiejie33botMatches.length;
          // console.log("tebiejie33botMatchesLength : " + tebiejie33botMatchesLength);  //测试
          if (tebiejie33botMatchesLength > 0) {
            for (let j = 0; j < tebiejie33botMatchesLength; j++) {
              if (tebiejie33botMatches[j]) {
                tebiejiebot.push(tebiejie33botMatches[j]);
              }
            }
          }
        }

        const tebiejie34botMatches = str.match(tebiejie34botRegexp);
        // console.log(tebiejie34botMatches);  //测试
        if (tebiejie34botMatches) {
          const tebiejie34botMatchesLength = tebiejie34botMatches.length;
          // console.log("tebiejie34botMatchesLength : " + tebiejie34botMatchesLength);  //测试
          if (tebiejie34botMatchesLength > 0) {
            for (let j = 0; j < tebiejie34botMatchesLength; j++) {
              if (tebiejie34botMatches[j]) {
                tebiejiebot.push(tebiejie34botMatches[j]);
              }
            }
          }
        }

        const tebiejie35botMatches = str.match(tebiejie35botRegexp);
        // console.log(tebiejie35botMatches);  //测试
        if (tebiejie35botMatches) {
          const tebiejie35botMatchesLength = tebiejie35botMatches.length;
          // console.log("tebiejie35botMatchesLength : " + tebiejie35botMatchesLength);  //测试
          if (tebiejie35botMatchesLength > 0) {
            for (let j = 0; j < tebiejie35botMatchesLength; j++) {
              if (tebiejie35botMatches[j]) {
                tebiejiebot.push(tebiejie35botMatches[j]);
              }
            }
          }
        }

        const tebiejie36botMatches = str.match(tebiejie36botRegexp);
        // console.log(tebiejie36botMatches);  //测试
        if (tebiejie36botMatches) {
          const tebiejie36botMatchesLength = tebiejie36botMatches.length;
          // console.log("tebiejie36botMatchesLength : " + tebiejie36botMatchesLength);  //测试
          if (tebiejie36botMatchesLength > 0) {
            for (let j = 0; j < tebiejie36botMatchesLength; j++) {
              if (tebiejie36botMatches[j]) {
                tebiejiebot.push(tebiejie36botMatches[j]);
              }
            }
          }
        }

        const tebiejie37botMatches = str.match(tebiejie37botRegexp);
        // console.log(tebiejie37botMatches);  //测试
        if (tebiejie37botMatches) {
          const tebiejie37botMatchesLength = tebiejie37botMatches.length;
          // console.log("tebiejie37botMatchesLength : " + tebiejie37botMatchesLength);  //测试
          if (tebiejie37botMatchesLength > 0) {
            for (let j = 0; j < tebiejie37botMatchesLength; j++) {
              if (tebiejie37botMatches[j]) {
                tebiejiebot.push(tebiejie37botMatches[j]);
              }
            }
          }
        }
        ...
        const MessengerIIbotMatches = str.match(MessengerIIbotRegexp);
        // console.log(MessengerIIbotMatches);  //测试
        if (MessengerIIbotMatches) {
          const MessengerIIbotMatchesLength = MessengerIIbotMatches.length;
          // console.log("MessengerIIbotMatchesLength : " + MessengerIIbotMatchesLength);  //测试
          if (MessengerIIbotMatchesLength > 0) {
            for (let j = 0; j < MessengerIIbotMatchesLength; j++) {
              if (MessengerIIbotMatches[j]) {
                MessengerIIbot.push(MessengerIIbotMatches[j]);
              }
            }
          }
        }

        const XVNgkllbotMatches = str.match(XVNgkllbotRegexp);
        // console.log(XVNgkllbotMatches);  //测试
        if (XVNgkllbotMatches) {
          const XVNgkllbotMatchesLength = XVNgkllbotMatches.length;
          // console.log("XVNgkllbotMatchesLength : " + XVNgkllbotMatchesLength);  //测试
          if (XVNgkllbotMatchesLength > 0) {
            for (let j = 0; j < XVNgkllbotMatchesLength; j++) {
              if (XVNgkllbotMatches[j]) {
                XVNgkllbot.push(XVNgkllbotMatches[j]);
              }
            }
          }
        }
        ...
        const FilesHubRobot1Matches = str.match(FilesHubRobot1Regexp);
        // console.log(FilesHubRobot1Matches);  //测试
        if (FilesHubRobot1Matches) {
          const FilesHubRobot1MatchesLength = FilesHubRobot1Matches.length;
          // console.log("FilesHubRobot1MatchesLength : " + FilesHubRobot1MatchesLength);  //测试
          if (FilesHubRobot1MatchesLength > 0) {
            for (let j = 0; j < FilesHubRobot1MatchesLength; j++) {
              if (FilesHubRobot1Matches[j]) {
                FilesHubRobot.push(FilesHubRobot1Matches[j]);
              }
            }
          }
        }

        const FilesHubRobot2Matches = str.match(FilesHubRobot2Regexp);
        // console.log(FilesHubRobot2Matches);  //测试
        if (FilesHubRobot2Matches) {
          const FilesHubRobot2MatchesLength = FilesHubRobot2Matches.length;
          // console.log("FilesHubRobot2MatchesLength : " + FilesHubRobot2MatchesLength);  //测试
          if (FilesHubRobot2MatchesLength > 0) {
            for (let j = 0; j < FilesHubRobot2MatchesLength; j++) {
              if (FilesHubRobot2Matches[j]) {
                FilesHubRobot.push(FilesHubRobot2Matches[j]);
              }
            }
          }
        }
        ...
        // const QQld90botMatches = str.match(QQld90botRegexp);
        // // console.log(QQld90botMatches);  //测试
        // if (QQld90botMatches) {
        //   const QQld90botMatchesLength = QQld90botMatches.length;
        //   // console.log("QQld90botMatchesLength : " + QQld90botMatchesLength);  //测试
        //   if (QQld90botMatchesLength > 0) {
        //     for (let j = 0; j < QQld90botMatchesLength; j++) {
        //       if (QQld90botMatches[j]) {
        //         QQfilebot.push(QQld90botMatches[j]);
        //       }
        //     }
        //   }
        // }

        const QQld90bot1Matches = str.match(QQld90bot1Regexp);
        // console.log(QQld90bot1Matches);  //测试
        if (QQld90bot1Matches) {
          const QQld90bot1MatchesLength = QQld90bot1Matches.length;
          // console.log("QQld90bot1MatchesLength : " + QQld90bot1MatchesLength);  //测试
          if (QQld90bot1MatchesLength > 0) {
            for (let j = 0; j < QQld90bot1MatchesLength; j++) {
              if (QQld90bot1Matches[j]) {
                QQfilebot.push(QQld90bot1Matches[j]);
              }
            }
          }
        }

        const QQld90bot2Matches = str.match(QQld90bot2Regexp);
        // console.log(QQld90bot2Matches);  //测试
        if (QQld90bot2Matches) {
          const QQld90bot2MatchesLength = QQld90bot2Matches.length;
          // console.log("QQld90bot2MatchesLength : " + QQld90bot2MatchesLength);  //测试
          if (QQld90bot2MatchesLength > 0) {
            for (let j = 0; j < QQld90bot2MatchesLength; j++) {
              if (QQld90bot2Matches[j]) {
                QQfilebot.push(QQld90bot2Matches[j]);
              }
            }
          }
        }

        const QQld90bot3Matches = str.match(QQld90bot3Regexp);
        // console.log(QQld90bot3Matches);  //测试
        if (QQld90bot3Matches) {
          const QQld90bot3MatchesLength = QQld90bot3Matches.length;
          // console.log("QQld90bot3MatchesLength : " + QQld90bot3MatchesLength);  //测试
          if (QQld90bot3MatchesLength > 0) {
            for (let j = 0; j < QQld90bot3MatchesLength; j++) {
              if (QQld90bot3Matches[j]) {
                QQfilebot.push(QQld90bot3Matches[j]);
              }
            }
          }
        }

        const QQld90bot4Matches = str.match(QQld90bot4Regexp);
        // console.log(QQld90bot4Matches);  //测试
        if (QQld90bot4Matches) {
          const QQld90bot4MatchesLength = QQld90bot4Matches.length;
          // console.log("QQld90bot4MatchesLength : " + QQld90bot4MatchesLength);  //测试
          if (QQld90bot4MatchesLength > 0) {
            for (let j = 0; j < QQld90bot4MatchesLength; j++) {
              if (QQld90bot4Matches[j]) {
                QQfilebot.push(QQld90bot4Matches[j]);
              }
            }
          }
        }

        const QQld90bot5Matches = str.match(QQld90bot5Regexp);
        // console.log(QQld90bot5Matches);  //测试
        if (QQld90bot5Matches) {
          const QQld90bot5MatchesLength = QQld90bot5Matches.length;
          // console.log("QQld90bot5MatchesLength : " + QQld90bot5MatchesLength);  //测试
          if (QQld90bot5MatchesLength > 0) {
            for (let j = 0; j < QQld90bot5MatchesLength; j++) {
              if (QQld90bot5Matches[j]) {
                QQfilebot.push(QQld90bot5Matches[j]);
              }
            }
          }
        }

        const QQld90bot6Matches = str.match(QQld90bot6Regexp);
        // console.log(QQld90bot6Matches);  //测试
        if (QQld90bot6Matches) {
          const QQld90bot6MatchesLength = QQld90bot6Matches.length;
          // console.log("QQld90bot6MatchesLength : " + QQld90bot6MatchesLength);  //测试
          if (QQld90bot6MatchesLength > 0) {
            for (let j = 0; j < QQld90bot6MatchesLength; j++) {
              if (QQld90bot6Matches[j]) {
                QQfilebot.push(QQld90bot6Matches[j]);
              }
            }
          }
        }

        const QQld90bot7Matches = str.match(QQld90bot7Regexp);
        // console.log(QQld90bot7Matches);  //测试
        if (QQld90bot7Matches) {
          const QQld90bot7MatchesLength = QQld90bot7Matches.length;
          // console.log("QQld90bot7MatchesLength : " + QQld90bot7MatchesLength);  //测试
          if (QQld90bot7MatchesLength > 0) {
            for (let j = 0; j < QQld90bot7MatchesLength; j++) {
              if (QQld90bot7Matches[j]) {
                QQfilebot.push(QQld90bot7Matches[j]);
              }
            }
          }
        }

        const yyjmq1Matches = str.match(yyjmq1Regexp);
        // console.log(yyjmq1Matches);  //测试
        if (yyjmq1Matches) {
          const yyjmq1MatchesLength = yyjmq1Matches.length;
          // console.log("yyjmq1MatchesLength : " + yyjmq1MatchesLength);  //测试
          if (yyjmq1MatchesLength > 0) {
            for (let j = 0; j < yyjmq1MatchesLength; j++) {
              if (yyjmq1Matches[j]) {
                yyjmq.push(yyjmq1Matches[j]);
              }
            }
          }
        }

        const yyjmq2Matches = str.match(yyjmq2Regexp);
        // console.log(yyjmq2Matches);  //测试
        if (yyjmq2Matches) {
          const yyjmq2MatchesLength = yyjmq2Matches.length;
          // console.log("yyjmq2MatchesLength : " + yyjmq2MatchesLength);  //测试
          if (yyjmq2MatchesLength > 0) {
            for (let j = 0; j < yyjmq2MatchesLength; j++) {
              if (yyjmq2Matches[j]) {
                yyjmq.push(yyjmq2Matches[j]);
              }
            }
          }
        }

        const yyjmq3Matches = str.match(yyjmq3Regexp);
        // console.log(yyjmq3Matches);  //测试
        if (yyjmq3Matches) {
          const yyjmq3MatchesLength = yyjmq3Matches.length;
          // console.log("yyjmq3MatchesLength : " + yyjmq3MatchesLength);  //测试
          if (yyjmq3MatchesLength > 0) {
            for (let j = 0; j < yyjmq3MatchesLength; j++) {
              if (yyjmq3Matches[j]) {
                yyjmq.push(yyjmq3Matches[j]);
              }
            }
          }
        }

        const yyjmq4Matches = str.match(yyjmq4Regexp);
        // console.log(yyjmq4Matches);  //测试
        if (yyjmq4Matches) {
          const yyjmq4MatchesLength = yyjmq4Matches.length;
          // console.log("yyjmq4MatchesLength : " + yyjmq4MatchesLength);  //测试
          if (yyjmq4MatchesLength > 0) {
            for (let j = 0; j < yyjmq4MatchesLength; j++) {
              if (yyjmq4Matches[j]) {
                yyjmq.push(yyjmq4Matches[j]);
              }
            }
          }
        }

        const yyjmq5Matches = str.match(yyjmq5Regexp);
        // console.log(yyjmq5Matches);  //测试
        if (yyjmq5Matches) {
          const yyjmq5MatchesLength = yyjmq5Matches.length;
          // console.log("yyjmq5MatchesLength : " + yyjmq5MatchesLength);  //测试
          if (yyjmq5MatchesLength > 0) {
            for (let j = 0; j < yyjmq5MatchesLength; j++) {
              if (yyjmq5Matches[j]) {
                yyjmq.push(yyjmq5Matches[j]);
              }
            }
          }
        }

        const yyjmq6Matches = str.match(yyjmq6Regexp);
        // console.log(yyjmq6Matches);  //测试
        if (yyjmq6Matches) {
          const yyjmq6MatchesLength = yyjmq6Matches.length;
          // console.log("yyjmq6MatchesLength : " + yyjmq6MatchesLength);  //测试
          if (yyjmq6MatchesLength > 0) {
            for (let j = 0; j < yyjmq6MatchesLength; j++) {
              if (yyjmq6Matches[j]) {
                yyjmq.push(yyjmq6Matches[j]);
              }
            }
          }
        }

        const yyjmq7Matches = str.match(yyjmq7Regexp);
        // console.log(yyjmq7Matches);  //测试
        if (yyjmq7Matches) {
          const yyjmq7MatchesLength = yyjmq7Matches.length;
          // console.log("yyjmq7MatchesLength : " + yyjmq7MatchesLength);  //测试
          if (yyjmq7MatchesLength > 0) {
            for (let j = 0; j < yyjmq7MatchesLength; j++) {
              if (yyjmq7Matches[j]) {
                yyjmq.push(yyjmq7Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot1Matches = str.match(yyjmqhbot1Regexp);
        // console.log(yyjmqhbot1Matches);  //测试
        if (yyjmqhbot1Matches) {
          const yyjmqhbot1MatchesLength = yyjmqhbot1Matches.length;
          // console.log("yyjmqhbot1MatchesLength : " + yyjmqhbot1MatchesLength);  //测试
          if (yyjmqhbot1MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot1MatchesLength; j++) {
              if (yyjmqhbot1Matches[j]) {
                yyjmqhbot.push(yyjmqhbot1Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot2Matches = str.match(yyjmqhbot2Regexp);
        // console.log(yyjmqhbot2Matches);  //测试
        if (yyjmqhbot2Matches) {
          const yyjmqhbot2MatchesLength = yyjmqhbot2Matches.length;
          // console.log("yyjmqhbot2MatchesLength : " + yyjmqhbot2MatchesLength);  //测试
          if (yyjmqhbot2MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot2MatchesLength; j++) {
              if (yyjmqhbot2Matches[j]) {
                yyjmqhbot.push(yyjmqhbot2Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot3Matches = str.match(yyjmqhbot3Regexp);
        // console.log(yyjmqhbot3Matches);  //测试
        if (yyjmqhbot3Matches) {
          const yyjmqhbot3MatchesLength = yyjmqhbot3Matches.length;
          // console.log("yyjmqhbot3MatchesLength : " + yyjmqhbot3MatchesLength);  //测试
          if (yyjmqhbot3MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot3MatchesLength; j++) {
              if (yyjmqhbot3Matches[j]) {
                yyjmqhbot.push(yyjmqhbot3Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot4Matches = str.match(yyjmqhbot4Regexp);
        // console.log(yyjmqhbot4Matches);  //测试
        if (yyjmqhbot4Matches) {
          const yyjmqhbot4MatchesLength = yyjmqhbot4Matches.length;
          // console.log("yyjmqhbot4MatchesLength : " + yyjmqhbot4MatchesLength);  //测试
          if (yyjmqhbot4MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot4MatchesLength; j++) {
              if (yyjmqhbot4Matches[j]) {
                yyjmqhbot.push(yyjmqhbot4Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot5Matches = str.match(yyjmqhbot5Regexp);
        // console.log(yyjmqhbot5Matches);  //测试
        if (yyjmqhbot5Matches) {
          const yyjmqhbot5MatchesLength = yyjmqhbot5Matches.length;
          // console.log("yyjmqhbot5MatchesLength : " + yyjmqhbot5MatchesLength);  //测试
          if (yyjmqhbot5MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot5MatchesLength; j++) {
              if (yyjmqhbot5Matches[j]) {
                yyjmqhbot.push(yyjmqhbot5Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot6Matches = str.match(yyjmqhbot6Regexp);
        // console.log(yyjmqhbot6Matches);  //测试
        if (yyjmqhbot6Matches) {
          const yyjmqhbot6MatchesLength = yyjmqhbot6Matches.length;
          // console.log("yyjmqhbot6MatchesLength : " + yyjmqhbot6MatchesLength);  //测试
          if (yyjmqhbot6MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot6MatchesLength; j++) {
              if (yyjmqhbot6Matches[j]) {
                yyjmqhbot.push(yyjmqhbot6Matches[j]);
              }
            }
          }
        }

        const yyjmqhbot7Matches = str.match(yyjmqhbot7Regexp);
        // console.log(yyjmqhbot7Matches);  //测试
        if (yyjmqhbot7Matches) {
          const yyjmqhbot7MatchesLength = yyjmqhbot7Matches.length;
          // console.log("yyjmqhbot7MatchesLength : " + yyjmqhbot7MatchesLength);  //测试
          if (yyjmqhbot7MatchesLength > 0) {
            for (let j = 0; j < yyjmqhbot7MatchesLength; j++) {
              if (yyjmqhbot7Matches[j]) {
                yyjmqhbot.push(yyjmqhbot7Matches[j]);
              }
            }
          }
        }

        const aotembotMatches = str.match(aotembotRegexp);
        // console.log(aotembotMatches);  //测试
        if (aotembotMatches) {
          const aotembotMatchesLength = aotembotMatches.length;
          // console.log("aotembotMatchesLength : " + aotembotMatchesLength);  //测试
          if (aotembotMatchesLength > 0) {
            for (let j = 0; j < aotembotMatchesLength; j++) {
              if (aotembotMatches[j]) {
                aotembot.push(aotembotMatches[j]);
              }
            }
          }
        }

        const jiematopbotMatches = str.match(jiematopbotRegexp);
        // console.log(jiematopbotMatches);  //测试
        if (jiematopbotMatches) {
          const jiematopbotMatchesLength = jiematopbotMatches.length;
          // console.log("jiematopbotMatchesLength : " + jiematopbotMatchesLength);  //测试
          if (jiematopbotMatchesLength > 0) {
            for (let j = 0; j < jiematopbotMatchesLength; j++) {
              if (jiematopbotMatches[j]) {
                jiematopbot.push(jiematopbotMatches[j]);
              }
            }
          }
        }

        const safsadfbotMatches = str.match(safsadfbotRegexp);
        // console.log(safsadfbotMatches);  //测试
        if (safsadfbotMatches) {
          const safsadfbotMatchesLength = safsadfbotMatches.length;
          // console.log("safsadfbotMatchesLength : " + safsadfbotMatchesLength);  //测试
          if (safsadfbotMatchesLength > 0) {
            for (let j = 0; j < safsadfbotMatchesLength; j++) {
              if (safsadfbotMatches[j]) {
                safsadfbot.push(safsadfbotMatches[j]);
              }
            }
          }
        }
        ...
        const bffilesbot1Matches = str.match(bffilesbot1Regexp);
        // console.log(bffilesbot1Matches);  //测试
        if (bffilesbot1Matches) {
          const bffilesbot1MatchesLength = bffilesbot1Matches.length;
          // console.log("bffilesbot1MatchesLength : " + bffilesbot1MatchesLength);  //测试
          if (bffilesbot1MatchesLength > 0) {
            for (let j = 0; j < bffilesbot1MatchesLength; j++) {
              if (bffilesbot1Matches[j]) {
                bffilesbot.push(bffilesbot1Matches[j]);
              }
            }
          }
        }

        const bffilesbot2Matches = str.match(bffilesbot2Regexp);
        // console.log(bffilesbot2Matches);  //测试
        if (bffilesbot2Matches) {
          const bffilesbot2MatchesLength = bffilesbot2Matches.length;
          // console.log("bffilesbot2MatchesLength : " + bffilesbot2MatchesLength);  //测试
          if (bffilesbot2MatchesLength > 0) {
            for (let j = 0; j < bffilesbot2MatchesLength; j++) {
              if (bffilesbot2Matches[j]) {
                bffilesbot.push(bffilesbot2Matches[j]);
              }
            }
          }
        }

        const bffilesbot3Matches = str.match(bffilesbot3Regexp);
        // console.log(bffilesbot3Matches);  //测试
        if (bffilesbot3Matches) {
          const bffilesbot3MatchesLength = bffilesbot3Matches.length;
          // console.log("bffilesbot3MatchesLength : " + bffilesbot3MatchesLength);  //测试
          if (bffilesbot3MatchesLength > 0) {
            for (let j = 0; j < bffilesbot3MatchesLength; j++) {
              if (bffilesbot3Matches[j]) {
                bffilesbot.push(bffilesbot3Matches[j]);
              }
            }
          }
        }

        const bffilesbot4Matches = str.match(bffilesbot4Regexp);
        // console.log(bffilesbot4Matches);  //测试
        if (bffilesbot4Matches) {
          const bffilesbot4MatchesLength = bffilesbot4Matches.length;
          // console.log("bffilesbot4MatchesLength : " + bffilesbot4MatchesLength);  //测试
          if (bffilesbot4MatchesLength > 0) {
            for (let j = 0; j < bffilesbot4MatchesLength; j++) {
              if (bffilesbot4Matches[j]) {
                bffilesbot.push(bffilesbot4Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot1Matches = str.match(KFCMNBbot1Regexp);
        // console.log(KFCMNBbot1Matches);  //测试
        if (KFCMNBbot1Matches) {
          const KFCMNBbot1MatchesLength = KFCMNBbot1Matches.length;
          // console.log("KFCMNBbot1MatchesLength : " + KFCMNBbot1MatchesLength);  //测试
          if (KFCMNBbot1MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot1MatchesLength; j++) {
              if (KFCMNBbot1Matches[j]) {
                KFCMNBbot.push(KFCMNBbot1Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot2Matches = str.match(KFCMNBbot2Regexp);
        // console.log(KFCMNBbot2Matches);  //测试
        if (KFCMNBbot2Matches) {
          const KFCMNBbot2MatchesLength = KFCMNBbot2Matches.length;
          // console.log("KFCMNBbot2MatchesLength : " + KFCMNBbot2MatchesLength);  //测试
          if (KFCMNBbot2MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot2MatchesLength; j++) {
              if (KFCMNBbot2Matches[j]) {
                KFCMNBbot.push(KFCMNBbot2Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot3Matches = str.match(KFCMNBbot3Regexp);
        // console.log(KFCMNBbot3Matches);  //测试
        if (KFCMNBbot3Matches) {
          const KFCMNBbot3MatchesLength = KFCMNBbot3Matches.length;
          // console.log("KFCMNBbot3MatchesLength : " + KFCMNBbot3MatchesLength);  //测试
          if (KFCMNBbot3MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot3MatchesLength; j++) {
              if (KFCMNBbot3Matches[j]) {
                KFCMNBbot.push(KFCMNBbot3Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot4Matches = str.match(KFCMNBbot4Regexp);
        // console.log(KFCMNBbot4Matches);  //测试
        if (KFCMNBbot4Matches) {
          const KFCMNBbot4MatchesLength = KFCMNBbot4Matches.length;
          // console.log("KFCMNBbot4MatchesLength : " + KFCMNBbot4MatchesLength);  //测试
          if (KFCMNBbot4MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot4MatchesLength; j++) {
              if (KFCMNBbot4Matches[j]) {
                KFCMNBbot.push(KFCMNBbot4Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot5Matches = str.match(KFCMNBbot5Regexp);
        // console.log(KFCMNBbot5Matches);  //测试
        if (KFCMNBbot5Matches) {
          const KFCMNBbot5MatchesLength = KFCMNBbot5Matches.length;
          // console.log("KFCMNBbot5MatchesLength : " + KFCMNBbot5MatchesLength);  //测试
          if (KFCMNBbot5MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot5MatchesLength; j++) {
              if (KFCMNBbot5Matches[j]) {
                KFCMNBbot.push(KFCMNBbot5Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot6Matches = str.match(KFCMNBbot6Regexp);
        // console.log(KFCMNBbot6Matches);  //测试
        if (KFCMNBbot6Matches) {
          const KFCMNBbot6MatchesLength = KFCMNBbot6Matches.length;
          // console.log("KFCMNBbot6MatchesLength : " + KFCMNBbot6MatchesLength);  //测试
          if (KFCMNBbot6MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot6MatchesLength; j++) {
              if (KFCMNBbot6Matches[j]) {
                KFCMNBbot.push(KFCMNBbot6Matches[j]);
              }
            }
          }
        }

        const KFCMNBbot7Matches = str.match(KFCMNBbot7Regexp);
        // console.log(KFCMNBbot7Matches);  //测试
        if (KFCMNBbot7Matches) {
          const KFCMNBbot7MatchesLength = KFCMNBbot7Matches.length;
          // console.log("KFCMNBbot7MatchesLength : " + KFCMNBbot7MatchesLength);  //测试
          if (KFCMNBbot7MatchesLength > 0) {
            for (let j = 0; j < KFCMNBbot7MatchesLength; j++) {
              if (KFCMNBbot7Matches[j]) {
                KFCMNBbot.push(KFCMNBbot7Matches[j]);
              }
            }
          }
        }

        const JSfilesbotMatches = str.match(JSfilesbotRegexp);
        // console.log(JSfilesbotMatches);  //测试
        if (JSfilesbotMatches) {
          const JSfilesbotMatchesLength = JSfilesbotMatches.length;
          // console.log("JSfilesbotMatchesLength : " + JSfilesbotMatchesLength);  //测试
          if (JSfilesbotMatchesLength > 0) {
            for (let j = 0; j < JSfilesbotMatchesLength; j++) {
              if (JSfilesbotMatches[j]) {
                JSfilesbot.push(JSfilesbotMatches[j]);
              }
            }
          }
        }

        const JSfilebotMatches = str.match(JSfilebotRegexp);
        // console.log(JSfilebotMatches);  //测试
        if (JSfilebotMatches) {
          const JSfilebotMatchesLength = JSfilebotMatches.length;
          // console.log("JSfilebotMatchesLength : " + JSfilebotMatchesLength);  //测试
          if (JSfilebotMatchesLength > 0) {
            for (let j = 0; j < JSfilebotMatchesLength; j++) {
              if (JSfilebotMatches[j]) {
                JSfilebot.push(JSfilebotMatches[j]);
              }
            }
          }
        }

        const JSfilessbotMatches = str.match(JSfilessbotRegexp);
        // console.log(JSfilessbotMatches);  //测试
        if (JSfilessbotMatches) {
          const JSfilessbotMatchesLength = JSfilessbotMatches.length;
          // console.log("JSfilessbotMatchesLength : " + JSfilessbotMatchesLength);  //测试
          if (JSfilessbotMatchesLength > 0) {
            for (let j = 0; j < JSfilessbotMatchesLength; j++) {
              if (JSfilessbotMatches[j]) {
                JSfilessbot.push(JSfilessbotMatches[j]);
              }
            }
          }
        }

        const JSfileessbotMatches = str.match(JSfileessbotRegexp);
        // console.log(JSfileessbotMatches);  //测试
        if (JSfileessbotMatches) {
          const JSfileessbotMatchesLength = JSfileessbotMatches.length;
          // console.log("JSfileessbotMatchesLength : " + JSfileessbotMatchesLength);  //测试
          if (JSfileessbotMatchesLength > 0) {
            for (let j = 0; j < JSfileessbotMatchesLength; j++) {
              if (JSfileessbotMatches[j]) {
                JSfileessbot.push(JSfileessbotMatches[j]);
              }
            }
          }
        }

        const JSfileeesbotMatches = str.match(JSfileeesbotRegexp);
        // console.log(JSfileeesbotMatches);  //测试
        if (JSfileeesbotMatches) {
          const JSfileeesbotMatchesLength = JSfileeesbotMatches.length;
          // console.log("JSfileeesbotMatchesLength : " + JSfileeesbotMatchesLength);  //测试
          if (JSfileeesbotMatchesLength > 0) {
            for (let j = 0; j < JSfileeesbotMatchesLength; j++) {
              if (JSfileeesbotMatches[j]) {
                JSfileeesbot.push(JSfileeesbotMatches[j]);
              }
            }
          }
        }

        const JScodefilebotMatches = str.match(JScodefilebotRegexp);
        // console.log(JScodefilebotMatches);  //测试
        if (JScodefilebotMatches) {
          const JScodefilebotMatchesLength = JScodefilebotMatches.length;
          // console.log("JScodefilebotMatchesLength : " + JScodefilebotMatchesLength);  //测试
          if (JScodefilebotMatchesLength > 0) {
            for (let j = 0; j < JScodefilebotMatchesLength; j++) {
              if (JScodefilebotMatches[j]) {
                JScodefilebot.push(JScodefilebotMatches[j]);
              }
            }
          }
        }
        ...
        const amumushuangxiangbotMatches = str.match(amumushuangxiangbotRegexp);
        // console.log(amumushuangxiangbotMatches);  //测试
        if (amumushuangxiangbotMatches) {
          const amumushuangxiangbotMatchesLength = amumushuangxiangbotMatches.length;
          // console.log("amumushuangxiangbotMatchesLength : " + amumushuangxiangbotMatchesLength);  //测试
          if (amumushuangxiangbotMatchesLength > 0) {
            for (let j = 0; j < amumushuangxiangbotMatchesLength; j++) {
              if (amumushuangxiangbotMatches[j]) {
                amumushuangxiangbot.push(amumushuangxiangbotMatches[j]);
              }
            }
          }
        }

        const tiantianziyoubotMatches = str.match(tiantianziyoubotRegexp);
        // console.log(tiantianziyoubotMatches);  //测试
        if (tiantianziyoubotMatches) {
          const tiantianziyoubotMatchesLength = tiantianziyoubotMatches.length;
          // console.log("tiantianziyoubotMatchesLength : " + tiantianziyoubotMatchesLength);  //测试
          if (tiantianziyoubotMatchesLength > 0) {
            for (let j = 0; j < tiantianziyoubotMatchesLength; j++) {
              if (tiantianziyoubotMatches[j]) {
                tiantianziyoubot.push(tiantianziyoubotMatches[j]);
              }
            }
          }
        }
  ...
  all += MessengerIIbot.length;
  console.log("MessengerIIbot : " + MessengerIIbot.length);  //测试
  if (MessengerIIbot.length > 0) {
    const data = fs.readFileSync("./code/MessengerIIbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...MessengerIIbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
  ...
  all += XVNgkllbot.length;
  console.log("XVNgkllbot : " + XVNgkllbot.length);  //测试
  if (XVNgkllbot.length > 0) {
    const data = fs.readFileSync("./code/XVNgkllbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...XVNgkllbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/XVNgkllbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += yyjmq.length;
  console.log("yyjmq : " + yyjmq.length);  //测试
  if (yyjmq.length > 0) {
    const data = fs.readFileSync("./code/yyjmq.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...yyjmq];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/yyjmq.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += yyjmqhbot.length;
  console.log("yyjmqhbot : " + yyjmqhbot.length);  //测试
  if (yyjmqhbot.length > 0) {
    const data = fs.readFileSync("./code/yyjmqhbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...yyjmqhbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/yyjmqhbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += aotembot.length;
  console.log("aotembot : " + aotembot.length);  //测试
  if (aotembot.length > 0) {
    const data = fs.readFileSync("./code/aotembot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...aotembot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
  ...
  all += jiematopbot.length;
  console.log("jiematopbot : " + jiematopbot.length);  //测试
  if (jiematopbot.length > 0) {
    const data = fs.readFileSync("./code/jiematopbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...jiematopbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
  ...
  all += safsadfbot.length;
  console.log("safsadfbot : " + safsadfbot.length);  //测试
  if (safsadfbot.length > 0) {
    const data = fs.readFileSync("./code/safsadfbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...safsadfbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
  ...
  all += bffilesbot.length;
  console.log("bffilesbot : " + bffilesbot.length);  //测试
  if (bffilesbot.length > 0) {
    const data = fs.readFileSync("./code/bffilesbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...bffilesbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/bffilesbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += KFCMNBbot.length;
  console.log("KFCMNBbot : " + KFCMNBbot.length);  //测试
  if (KFCMNBbot.length > 0) {
    const data = fs.readFileSync("./code/KFCMNBbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...KFCMNBbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
  ...
  all += JSfilesbot.length;
  console.log("JSfilesbot : " + JSfilesbot.length);  //测试
  if (JSfilesbot.length > 0) {
    const data = fs.readFileSync("./code/JSfilesbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...JSfilesbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/JSfilesbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += JSfilebot.length;
  console.log("JSfilebot : " + JSfilebot.length);  //测试
  if (JSfilebot.length > 0) {
    const data = fs.readFileSync("./code/JSfilebot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...JSfilebot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
  ...
  all += JSfilessbot.length;
  console.log("JSfilessbot : " + JSfilessbot.length);  //测试
  if (JSfilessbot.length > 0) {
    const data = fs.readFileSync("./code/JSfilessbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...JSfilessbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/JSfilessbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += JSfileessbot.length;
  console.log("JSfileessbot : " + JSfileessbot.length);  //测试
  if (JSfileessbot.length > 0) {
    const data = fs.readFileSync("./code/JSfileessbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...JSfileessbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/JSfileessbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += JSfileeesbot.length;
  console.log("JSfileeesbot : " + JSfileeesbot.length);  //测试
  if (JSfileeesbot.length > 0) {
    const data = fs.readFileSync("./code/JSfileeesbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...JSfileeesbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/JSfileeesbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += JScodefilebot.length;
  console.log("JScodefilebot : " + JScodefilebot.length);  //测试
  if (JScodefilebot.length > 0) {
    const data = fs.readFileSync("./code/JScodefilebot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...JScodefilebot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/JScodefilebot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += amumushuangxiangbot.length;
  console.log("amumushuangxiangbot : " + amumushuangxiangbot.length);  //测试
  if (amumushuangxiangbot.length > 0) {
    const data = fs.readFileSync("./code/amumushuangxiangbot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...amumushuangxiangbot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/amumushuangxiangbot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
  ...
  all += tiantianziyoubot.length;
  console.log("tiantianziyoubot : " + tiantianziyoubot.length);  //测试
  if (tiantianziyoubot.length > 0) {
    const data = fs.readFileSync("./code/tiantianziyoubot.txt", "utf-8");
    try {
      let uniqueArr = JSON.parse(data);
      const oldLength = uniqueArr.length;
      uniqueArr = [...uniqueArr, ...tiantianziyoubot];
      uniqueArr = [...new Set(uniqueArr)];
      if (uniqueArr.length > oldLength) {
        fs.writeFile("./code/tiantianziyoubot.txt", JSON.stringify(uniqueArr, null, 2), function(err) {
          if (err) {
            console.log(err instanceof Error ? err.message : err);
          }
        });
