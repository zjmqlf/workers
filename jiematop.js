...
export class WebSocketServer extends DurableObject {
  ...
  async getMessage(tryCount) {
    try {
      ...
      if (err.errorMessage?.includes("FLOOD_WAIT_") === true || err.code === 420) {
        ...
        this.sendLog("getMessage", "出错 : " + err instanceof Error ? err.message : err, null, true);
        if (tryCount === 5) {
          this.stop = 2;
        ...
        if (status) {
          ...
          try {
            ...
            if (err.errorMessage?.includes("FLOOD_WAIT_") === true || err.code === 420) {
              ...
              this.sendLog("sendQuery", "出错 : " + err instanceof Error ? err.message : err, "error", true);
    ...
    try {
      users = await this.client.invoke(
        new Api.users.GetUsers({
          id: [
            new Api.InputUser({
              userId: bigInt("8908766090"),
              accessHash: bigInt("4697995986322785712"),
            }),
          ],
        })
      );
      ...
      this.sendLog("getBot", "出错 : " + err instanceof Error ? err.message : err, null, true);
  ...
  async getUser(tryCount) {
    ...
    try {
      ...
      this.sendLog("getUser", "出错 : " + err instanceof Error ? err.message : err, null, true);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/ws") {
      ...
      const id = env.WEBSOCKET_SERVER.idFromName("jiematop");
